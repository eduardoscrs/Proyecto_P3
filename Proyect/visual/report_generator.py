...
def generate_pdf_report(sim_data, avl_tree):
    ...
    with PdfPages(buffer) as pdf:
        ...
        top_routes = avl_tree.get_top_routes(10)
        y = 0.9
        ...
        client_orders = {cid: c.total_orders for cid, c in clients.items() if hasattr(c, "total_orders")}
        ...
        axs = axs.flatten()
        recharge_usage = {n: sim_data["nx_graph"].degree(n) for n in sim_data["recharge_nodes"]}
        pd.Series(recharge_usage).sort_values(ascending=False).head(5).plot(kind="bar", ax=axs[0], color="blue", title="🔋 Recarga")
        storage_usage = {n: sim_data["nx_graph"].degree(n) for n in sim_data["storage_nodes"]}
        pd.Series(storage_usage).sort_values(ascending=False).head(5).plot(kind="bar", ax=axs[1], color="orange", title="📦 Almacenamiento")
        client_usage = {n: sim_data["nx_graph"].degree(n) for n in sim_data["client_nodes"]}
